...
async function getOpenjdkBuildResource(usePPRef: Boolean): Promise<void> {
  if (!usePPRef) {
    await exec.exec(`git clone --depth 1 --branch travaopenjdk-ga https://github.com/SyedAnas26/temurin-build-dcevm.git`)
  }
}
...
async function getBootJdk(bootJDKVersion: string, impl: string): Promise<string> {
    ...
    if (
      ...
      bootjdkJar = await tc.downloadTool(`https://api.adoptopenjdk.net/v3/binary/latest/${bootJDKVersion}/ga/${targetOs}/x64/jdk/hotspot/normal/adoptopenjdk`)
    }
